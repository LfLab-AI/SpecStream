...
[[ "$FIXED_DRAFT_TPCS" == 27 ]] || {
  echo "ERROR: this manual requires FIXED_DRAFT_TPCS=27" >&2
  exit 2
}
...
for rep in 1 2 3 4 5; do
  ...
  for method in "${methods[@]}"; do
    ...
    export BENCH_CMD="TARGET_MODEL='$TARGET_MODEL' DATASET_NAME=random-ids INPUT_LEN=16384 OUTPUT_LEN=128 NUM_PROMPTS=64 REQUEST_RATE=inf MAX_CONCURRENCY=8 RANGE_RATIO=1 WARMUP_REQUESTS=4 SEED=1 CONTEXT_LEN=$CONTEXT_LENGTH CASE_TAG=${method}_ctx16k_c8_fixed27_r${rep} OUTPUT_DIR='$TEST_ROOT/performance/$method/results' RUN_ID=r${rep} bash '$BENCH_RUNNER'"

    echo "RUN rep=$rep method=$method fixed_tpcs=$FIXED_DRAFT_TPCS"
    bash "$CASE_RUNNER"

    if [[ "$method" == C ]]; then
      grep -F -- '--specstream-smctrl-calibration-tpcs 27' \
        "$CASE_ROOT/target_command.txt" >/dev/null || {
          echo "ERROR: C did not use fixed 27 TPCs" >&2
          exit 2
        }
      grep -F -- '--specstream-smctrl-calibration-allow-overlap' \
        "$CASE_ROOT/target_command.txt" >/dev/null || {
      ...
      "$SPECSTREAM_PYTHON" "$ACK_ANALYZER" \
        ...
        --output "$TEST_ROOT/summary/performance_C_r${rep}_fixed27_ack.json" \
        --require-complete
    fi

    echo "PASS rep=$rep method=$method"
  done
done

echo "ALL_15_FIXED27_CASES=PASS"

...
case "$DATASET_NAME" in
  random-ids)
    args+=(--tokenize-prompt --num-prompts "$NUM_PROMPTS" \
      --random-input-len "$INPUT_LEN" --random-output-len "$OUTPUT_LEN" \
      --random-range-ratio "$RANGE_RATIO")
    ;;
  random)
    args+=(--num-prompts "$NUM_PROMPTS" --random-input-len "$INPUT_LEN" \
      --random-output-len "$OUTPUT_LEN" --random-range-ratio "$RANGE_RATIO")
    [[ -n "$DATASET_PATH" ]] && args+=(--dataset-path "$DATASET_PATH")
    ;;
  sharegpt|custom|longbench_v2)
    args+=(--num-prompts "$NUM_PROMPTS" --sharegpt-output-len "$OUTPUT_LEN" \
      --sharegpt-context-len "$CONTEXT_LEN")
    [[ -n "$DATASET_PATH" ]] && args+=(--dataset-path "$DATASET_PATH")
    ;;
  *) echo "ERROR: unsupported DATASET_NAME=$DATASET_NAME" >&2; exit 3 ;;
esac

echo "[BENCH] python=$SPECSTREAM_PYTHON"
echo "[BENCH] case=$CASE_TAG dataset=$DATASET_NAME input=$INPUT_LEN output=$OUTPUT_LEN C=$MAX_CONCURRENCY"
"${args[@]}"

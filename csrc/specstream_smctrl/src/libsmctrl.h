...
// Uses the QMD/TMD callback path. Validate on the exact driver before use;
// unlike the per-stream path it does not depend on a stream-struct offset.
...
// @param mask   A bitmask of enabled/disabled TPCs (see Notes on Bitmasks)
// Supported offsets: CUDA 8.0 - CUDA 12.8
extern int libsmctrl_set_stream_mask(void* stream, uint64_t mask);
extern int libsmctrl_set_stream_mask_ext(void* stream, uint128_t mask);
...
// @param mask   A bitmask of enabled/disabled TPCs (see Notes on Bitmasks)
// Uses the same QMD/TMD callback path as the global mask.
extern int libsmctrl_set_next_mask(uint64_t mask);
...
// create tpc mask
extern int libsmctrl_make_mask(uint64_t *result, uint32_t low, uint32_t high_exlusive);
// create tpc mask
extern int libsmctrl_make_mask_ext(uint128_t *result, uint32_t low, uint32_t high_exlusive);
// validate stream mask exactly works
extern int libsmctrl_validate_stream_mask(void *stream_ptr, int low, int high, bool echo);

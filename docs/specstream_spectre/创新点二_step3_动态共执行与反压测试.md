...
本步只测试创新点二：Target 继续使用原生 SPECTRE 的全 GPU KV。`--specstream-profile-only` 用来启动动态 q 和卡内共执行控制器，但不会启用创新点一的 KV 卸载。
...
mkdir -p logs/innovation2_step3 results/innovation2_step3 profiles/innovation2_step3
...
python -m sglang.launch_server \
  ...
  --disable-radix-cache --disable-cuda-graph --disable-overlap-schedule \
  --spectre-zmq-addr 127.0.0.1 --spectre-zmq-port 29000 \
  2>&1 | tee logs/innovation2_step3/O1_draft.log
```
...
python -m sglang.bench_serving \
  ...
  --tag I2-S3-O1-smoke \
  --output-file results/innovation2_step3/O1_smoke.jsonl
```
...
python -m sglang.bench_serving \
  ...
  --tag I2-S3-O1-16k-c8 \
  --output-file results/innovation2_step3/O1_16k_c8.jsonl
```
...
python -m sglang.bench_serving \
  ...
  --warmup-requests 8 --seed 1 --flush-cache --output-details \
  --tag I2-S3-O1-poisson-r4 \
  --output-file results/innovation2_step3/O1_poisson_r4.jsonl
```

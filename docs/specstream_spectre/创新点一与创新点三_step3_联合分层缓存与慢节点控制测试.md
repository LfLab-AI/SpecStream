# 创新点一与创新点三 step3：分层缓存与多卡慢节点控制联合测试
...
4. CPU KV 传输与 Draft 共卡同时存在时，控制器能否降低 TP 慢节点影响。

本步同时启用创新点一和创新点三：Target 使用 CPU/GPU 分层 KV，并运行共卡保护及 TP 慢节点控制。
...
mkdir -p logs/innovation13_step3 results/innovation13_step3 profiles/innovation13_step3
...
python -m sglang.launch_server \
  ...
  --specstream-profile-path profiles/innovation13_step3/O1_tp_control.csv \
  --disable-radix-cache --disable-cuda-graph --disable-overlap-schedule \
  --spectre-zmq-addr 127.0.0.1 --spectre-zmq-port 29000 \
  2>&1 | tee logs/innovation13_step3/O1_target.log
```
...
python -m sglang.launch_server \
  ...
  --disable-radix-cache --disable-cuda-graph --disable-overlap-schedule \
  --spectre-zmq-addr 127.0.0.1 --spectre-zmq-port 29000 \
  2>&1 | tee logs/innovation13_step3/O1_draft.log
```
...
python -m sglang.bench_serving \
  ...
  --tag I13-S3-O1-smoke \
  --output-file results/innovation13_step3/O1_smoke.jsonl
```
...
python -m sglang.bench_serving \
  ...
  --tag I13-S3-O1-16k-c8 \
  --output-file results/innovation13_step3/O1_16k_c8.jsonl
```
...
python scripts/specstream/summarize_specstream_profile.py \
  'profiles/innovation13_step3/*.csv' \
  > results/innovation13_step3/profile_summary.tsv

python scripts/specstream/summarize_benchmarks.py \
  'results/innovation13_step3/*.jsonl' \
  > results/innovation13_step3/benchmark_summary.tsv
```

目的：把分层 KV 分卡 P0、分层 KV 朴素共卡 P1、分层 KV 加创新点三控制 O1 放在同一结果表中，比较总吞吐、P99、`goodput/GPU`、rank skew、Target slowdown、H2D 和回退次数。
...
5. P0、P1、O1 的 `cpu_history_bytes`、`h2d_bytes` 和 `h2d_ops` 必须大于 0；否则说明创新点一没有真正运行。

### 可选联合完整组：增加分块分组

停止 O1 后，在第 4 节 Target 命令中增加：
...
将 profile、tag、输出文件名改为 `O2_cohort`。目的：判断 Cohort 能否在不扩大 rank skew 和 P99 的情况下减少 H2D 操作次数。

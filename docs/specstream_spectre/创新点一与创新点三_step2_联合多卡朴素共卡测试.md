# 创新点一与创新点三 step2：分层缓存下的多卡朴素共卡测试
...
本步启用创新点一的分层 KV，但不开启 TP 慢节点保护。它用来复现“CPU/GPU KV 传输 + Draft 共卡”共同作用时 rank 0 变慢的情况，是联合优化组的直接对照。
...
nvidia-smi -L
mkdir -p logs/innovation13_step2 results/innovation13_step2 profiles/innovation13_step2
...
python -m sglang.launch_server \
  ...
  --specstream-profile-path profiles/innovation13_step2/P1_naive_80_20.csv \
  --disable-radix-cache --disable-cuda-graph --disable-overlap-schedule \
  --spectre-zmq-addr 127.0.0.1 --spectre-zmq-port 29000 \
  2>&1 | tee logs/innovation13_step2/P1_target_80_20.log
```
...
python -m sglang.launch_server \
  ...
  --disable-radix-cache --disable-cuda-graph --disable-overlap-schedule \
  --spectre-zmq-addr 127.0.0.1 --spectre-zmq-port 29000 \
  2>&1 | tee logs/innovation13_step2/P1_draft_80_20.log
```
...
python -m sglang.bench_serving \
  ...
  --tag I13-S2-P1-80-20-smoke \
  --output-file results/innovation13_step2/P1_80_20_smoke.jsonl
```
...
python -m sglang.bench_serving \
  ...
  --tag I13-S2-P1-80-20-16k-c8 \
  --output-file results/innovation13_step2/P1_80_20_16k_c8.jsonl
```
...
nvidia-smi dmon -s pucvmet -d 1 \
  -o DT > results/innovation13_step2/nvidia_smi_dmon.log
```
...
python scripts/specstream/summarize_specstream_profile.py \
  'profiles/innovation13_step2/*.csv' \
  > results/innovation13_step2/profile_summary.tsv

python scripts/specstream/summarize_benchmarks.py \
  'results/innovation13_step2/*.jsonl' \
  > results/innovation13_step2/benchmark_summary.tsv
```

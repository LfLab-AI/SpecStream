# 创新点三 step1：目标模型多卡并行与独立草稿模型强基线测试
...
本步不启用创新点一，也不启用创新点二、三的控制器。`--specstream-profile-only` 仅为每个 TP rank 记录可比较的时间数据；Target KV 全部驻留 GPU。
...
mkdir -p logs/innovation3_step1 results/innovation3_step1 profiles/innovation3_step1
```
...
CUDA_VISIBLE_DEVICES=0,1 python -m sglang.launch_server \
  ...
  --specstream-profile-path profiles/innovation3_step1/P0_tp2.csv \
  --disable-radix-cache --disable-cuda-graph --disable-overlap-schedule \
  --spectre-zmq-addr 127.0.0.1 --spectre-zmq-port 29000 \
  2>&1 | tee logs/innovation3_step1/P0_target.log
```

目的：让原生 GPU-KV Target 使用 GPU0/GPU1 做 TP=2。该命令不包含动态 q、卡内共执行或 TP 慢节点控制。
...
CUDA_VISIBLE_DEVICES=2 python -m sglang.launch_server \
  ...
  --disable-radix-cache --disable-cuda-graph --disable-overlap-schedule \
  --spectre-zmq-addr 127.0.0.1 --spectre-zmq-port 29000 \
  2>&1 | tee logs/innovation3_step1/P0_draft.log
```
...
python -m sglang.bench_serving \
  ...
  --tag I3-S1-P0-smoke \
  --output-file results/innovation3_step1/P0_smoke.jsonl
```
...
python -m sglang.bench_serving \
  ...
  --tag I3-S1-P0-16k-c8 \
  --output-file results/innovation3_step1/P0_16k_c8.jsonl
```
...
python -m sglang.bench_serving \
  ...
  --tag I3-S1-P0-30k-c4 \
  --output-file results/innovation3_step1/P0_30k_c4.jsonl
```
...
python scripts/specstream/summarize_specstream_profile.py \
  'profiles/innovation3_step1/*.csv' \
  > results/innovation3_step1/profile_summary.tsv

python scripts/specstream/summarize_benchmarks.py \
  'results/innovation3_step1/*.jsonl' \
  > results/innovation3_step1/benchmark_summary.tsv
```

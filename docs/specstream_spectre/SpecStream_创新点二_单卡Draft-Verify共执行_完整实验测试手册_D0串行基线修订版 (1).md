...
2. 把 Draft/Verify 改成同卡并行后（D2），是否真的优于 D0，还是资源争用抵消了并行收益？
3. 固定 TPC 隔离是否存在安全 Pareto 点？
4. 在线 Target-priority grant 是否能把 Target slowdown 保持在 5% 预算内，并进一步优于 D0/D2/D3？
5. 与两卡 SPECTRE 相比，单卡 raw throughput 损失多少，但 throughput/GPU 与资源效率改善多少？
...
|---|---:|---|---|---|
| D0 | 1 | GPU-resident | **原生 SGLang STANDALONE：Draft→Target Verify 串行执行** | 单卡传统推测解码基线，回答“并行本身是否值得” |
...
```text
单卡因果主线：D0（串行） → D2（同卡 naive 并行） → D3（固定 TPC） → D4（在线控制）
双卡强参考：D1（Target/Draft 分卡并行）
```

其中 D0 是创新点二最重要的单卡串行参考：D0→D2 用于判断“把 Draft 与 Verify 并行化本身是否带来收益”，D0→D4 用于判断“受控共执行相较传统串行推测解码是否真正有价值”。D5 只用于最终完整系统协同实验。
...
# 15. Gate B：libsmctrl（D1 完成后，为 D2–D4 单卡阶段准备）
...
# 19. Gate D：first-grant-before-first-forward smoke
...
# 29. D4：online Target-priority TPC grant
...
| Method | GPUs | Execution | Raw Output Throughput | Throughput/GPU | P99 TTFT | P99 TPOT | P99 E2E | Target Slowdown |
|---|---:|---|---:|---:|---:|---:|---:|---:|
| **D0 SGLang STANDALONE** | 1 | **Serial Draft→Verify** | | | | | | N/A |
| D1 2-GPU SPECTRE | 2 | Parallel / separate GPUs | | | | | | |
| D2 Same-GPU uncontrolled | 1 | Parallel / no TPC control | | | | | | |
| D3 Static TPC | 1 | Parallel / fixed TPC | | | | | | |
| D4 Online Target-priority | 1 | Online one-token grant | | | | | | |

D0 现在是创新点二主表中的第一类基线，而不是辅助参考。D5 放完整系统表，不与 I2 isolation 主表混在一起。
...
[2] D0：原生 SGLang STANDALONE，单卡串行 Draft→Verify
        ↓
[3] first-grant / SpecStream 单测
        ↓
[4] D1：双卡 SPECTRE parallel q4（此时不启用 MPS）
        ↓
[5] libsmctrl build + validate-global
        ↓
[6] 启动单卡 MPS
        ↓
[7] D2：同卡 q4，无 TPC 控制
        ↓
[8] Gate D：TPC=4 / C1 / 4 requests
        ↓
[9] Gate 成功后 TPC feasibility scan
        ↓
[10] 统计真实 batch/q/context shape
        ↓
[11] 构建 >=95% coverage measured profile
        ↓
[12] D3：最佳 fixed TPC
        ↓
[13] 验证 coexec_runtime online grant/ACK
        ↓
[14] D4：online Target-priority
        ↓
[15] D4 profile coverage >=95%
        ↓
[16] 扩展 context/concurrency/rate sweep
        ↓
[17] 5×>=1000-request confirmatory runs
        ↓
[18] D5 full I1+I2
```
...
> 在当前 workload 下，同卡 Draft–Verify 并行本身相较传统串行 speculative decoding 有正收益；后续才有必要讨论如何通过 TPC/grant 控制这种共执行。
...
> 当前配置下并行化收益很小；即使 TPC 控制有效，端到端提升上限也可能有限，应进一步提高并发、Draft 压力或更换 Draft/Target 组合寻找真正的 contention region。
...
> naive 同卡并行产生的资源争用已经超过并行收益，此时 D3/D4 的目标应首先是恢复甚至超过 D0，而不是只与 D2 比较。
...
> **先用 D0 建立传统单卡串行基线，再判断 D2 的并行化本身是否值得；随后才讨论 TPC、first-grant、measured profile 与 online grant。先证明因果链，再讨论性能。**

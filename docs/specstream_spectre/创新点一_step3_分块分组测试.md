...
当前实现的重要边界：cohort 对当前 verification batch 中已经到达的工作进行分组，并不会主动 sleep 等待未来请求。已经进入同一 verify batch 的请求不会再因为 Python 构造 descriptor 超过 200 us 而被拆成 singleton；因此 `max_cohort_delay_us` 当前不形成真实的主动等待曲线。论文若要声称跨调度周期等待/聚合，需要另行实现真正的 pending queue。
...
`--specstream-layer-prefetch` 在本版本不再局限于 `len(stream_items)==1`。C0 使用跨请求扁平 transfer 队列，C1 使用跨 cohort-plan 扁平队列；两者都只保留两个 bounded staging slot。cohort 的跨层预取使用 direct-async H2D 写入 GPU cohort staging，不再在 layer L 的 Tail/MLP 入队前同步复制几百 MiB 的 CPU pack buffer。steady-state transfer 小于 32 MiB 时仍使用单次 packed DMA；达到 32 MiB 后自动切换为 direct async pinned-slab H2D，避免同步 CPU 重打包成为高并发关键路径。日志会出现 `SpecStream cohort uses direct async H2D above 32 MiB`。c>1 首次真正消费预取数据时还必须出现 `SpecStream multi-request layer prefetch active: mode=independent|cohort`。最终仍应通过 Nsight Systems 确认下一层 H2D 与当前层 Tail/MLP 存在重叠。

### 5.3 固定 q 的 ordinary/parallel 成对隔离组

上面 C0/C1 开启的是联合 `(mode,q)` 控制器，它可以根据成本主动选择 ordinary。为了分离 Draft/Verify overlap，固定 q=5 必须成对运行 ordinary 和 parallel，而不能只保留 parallel。先从 C0/C1 命令删除以下三项：
...
# 完全重启 Target 和 Drafter后的第二轮
--spectre-fixed-q-mode parallel
```

Drafter 与 Target 都保持 `--speculative-num-steps 4 --speculative-num-draft-tokens 5`，并加入 `--spectre-failure-threshold 3 --spectre-cooldown-rounds 32`。C0 不加 cohort 三个参数；C1 保留 `--specstream-cohort-enabled --specstream-max-cohort-size 8 --specstream-max-cohort-delay-us 200`。profile 文件名必须包含 `ordinary` 或 `parallel`。

parallel 结果的有效门槛是：每个并发点的 Accept length 与 ordinary 接近，Target 没有持续 `[DraftFallback]`，profile 的 missing-draft ratio 接近 0。若 c=16 像本轮结果一样从约 4.2 突然变为 1.0，退化主因是 remote draft timeout，而不是 attention kernel；应报告 ordinary/dynamic 结果，并单独修复 Drafter 容量。为了避免一次实验被 30 次连续超时拖慢，新默认在 3 次失败后打开 circuit，32 个 q=1 round 后才探测恢复。

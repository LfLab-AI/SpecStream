# 创新点一 step1：目标模型分层缓存、分块流式传输与在线归一化测试
...
下面先给出 B0-B3 的 `parallel` 性能命令：Target 验证当前 round 时，CPU 同时接收 Drafter 的下一轮草稿。但 parallel 不是无条件更快：只有日志无持续 timeout、Accept length 没有退化为 1 时，该结果才有效。c=16/32 必须同时补跑 ordinary 或 dynamic `(mode,q)`；若 parallel 发生 timeout，不能用它评价 SpecStream kernel。第一次确认 q>1 链路和 shadow 正确性时，也应临时改成 `ordinary`。

这里的 `--disable-overlap-schedule` 关闭的是 SGLang 通用 overlap scheduler，并不等于关闭 SPECTRE 的远程 Draft/Target overlap；后者由 `--spectre-fixed-q-mode parallel` 控制。为了 B0-B3 公平，通用 scheduler 开关保持一致。
...
CUDA_VISIBLE_DEVICES=1 python -m sglang.launch_server \
  ...
  --disable-radix-cache --disable-cuda-graph --disable-overlap-schedule \
  --spectre-zmq-addr 127.0.0.1 --spectre-zmq-port 29000 --specstream-layer-prefetch \
...
多并发下 `--specstream-layer-prefetch` 现在也会生效：非 cohort 路径把所有请求的 transfer group 展平成一条全局双缓冲队列；cohort 路径把所有 cohort plan 展平成一条全局队列。任何时刻仍然最多只预取 `--specstream-num-buffers` 组，不会按请求数额外分配 staging。C=16/32 的最终实验还应按 Step 3 加入 `--specstream-cohort-enabled`，使跨请求 History 真正进入 packed DMA 和 batched cohort kernel。

若要做一次 ordinary 正确性校准，只修改以下两项并使用单并发：
...
确认 Target profile 中实际 q>1、无 stale/timeout 且 shadow 通过后，必须恢复本节的 `parallel + fallback` 再记录性能。

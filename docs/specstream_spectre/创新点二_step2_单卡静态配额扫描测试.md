...
本步也不开启 `--specstream-enabled`：所有 Target KV 均为原生 SPECTRE GPU KV。`--specstream-profile-only` 只记录指标，不进行创新点一的 CPU 卸载。
...
nvidia-smi -L
mkdir -p logs/innovation2_step2 results/innovation2_step2 profiles/innovation2_step2
...
python -m sglang.launch_server \
  ...
  --specstream-profile-path profiles/innovation2_step2/static_80_20.csv \
  --disable-radix-cache --disable-cuda-graph --disable-overlap-schedule \
  --spectre-zmq-addr 127.0.0.1 --spectre-zmq-port 29000 \
  2>&1 | tee logs/innovation2_step2/static_80_20_target.log
```
...
python -m sglang.launch_server \
  ...
  --disable-radix-cache --disable-cuda-graph --disable-overlap-schedule \
  --spectre-zmq-addr 127.0.0.1 --spectre-zmq-port 29000 \
  2>&1 | tee logs/innovation2_step2/static_80_20_draft.log
```
...
python -m sglang.bench_serving \
  ...
  --tag I2-S2-static-80-20-smoke \
  --output-file results/innovation2_step2/static_80_20_smoke.jsonl
```
...
python -m sglang.bench_serving \
  ...
  --tag I2-S2-static-80-20-16k-c8 \
  --output-file results/innovation2_step2/static_80_20_16k_c8.jsonl
```
...
python -m sglang.bench_serving \
  ...
  --tag I2-S2-static-80-20-30k-c4 \
  --output-file results/innovation2_step2/static_80_20_30k_c4.jsonl
```
...
nvidia-smi dmon -s pucvmet -d 1 \
  -o DT > results/innovation2_step2/nvidia_smi_dmon.log
```

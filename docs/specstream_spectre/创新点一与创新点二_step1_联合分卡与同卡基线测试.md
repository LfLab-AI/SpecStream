# 创新点一与创新点二 step1：分层缓存下的分卡基线与同卡无控制对照

## 1. 本步测试目的

本步建立两个参照组：

- `I12-S1-B0`：Target 在 GPU 0，Draft 在 GPU 1。它是没有同卡资源竞争的强 baseline。
- `I12-S1-B1`：Target 和 Draft 都在 GPU 0，不启用 MPS，也不开启卡内控制。它用于测量直接共卡造成的干扰。

本文件中的命令均为直接执行方式，不需要设置 `SPECSTREAM_TARGET_CMD`、`SPECSTREAM_DRAFT_CMD` 或 `SPECSTREAM_BENCH_CMD`。

本文件测试“创新点一的 KV 分层卸载 + 创新点二的卡内部署”。Target 的较老 KV 会保存在 CPU，近期 KV 保留在 GPU。step1 暂时不开启动态共执行控制，只建立分卡和无控制同卡两组联合基线。
...
mkdir -p logs/innovation12_step1 results/innovation12_step1 profiles/innovation12_step1
```
...
## 4. I12-S1-B0：Target GPU 0、Draft GPU 1
...
CUDA_VISIBLE_DEVICES=0 python -m sglang.launch_server \
  ...
  --specstream-profile-path profiles/innovation12_step1/B0_dedicated.csv \
  --disable-radix-cache --disable-cuda-graph --disable-overlap-schedule \
  --spectre-zmq-addr 127.0.0.1 --spectre-zmq-port 29000 \
  2>&1 | tee logs/innovation12_step1/B0_target.log
```
...
CUDA_VISIBLE_DEVICES=1 python -m sglang.launch_server \
  ...
  --disable-radix-cache --disable-cuda-graph --disable-overlap-schedule \
  --spectre-zmq-addr 127.0.0.1 --spectre-zmq-port 29000 \
  2>&1 | tee logs/innovation12_step1/B0_draft.log
```
...
python -m sglang.bench_serving \
  ...
  --tag I12-S1-B0-smoke \
  --output-file results/innovation12_step1/B0_smoke.jsonl
```
...
python -m sglang.bench_serving \
  ...
  --tag I12-S1-B0-16k-c8 \
  --output-file results/innovation12_step1/B0_16k_c8.jsonl
```

目的：建立分卡 baseline 的主要吞吐和延迟数据。之后把 `--max-concurrency 8` 分别改为 `1、4、16、32`，每个并发点完整重启 Target 和 Draft，并修改输出文件名。

## 5. I12-S1-B1：Target 与 Draft 同卡、无控制
...
```bash
--specstream-profile-path profiles/innovation12_step1/B1_uncontrolled.csv
```
...
```bash
2>&1 | tee logs/innovation12_step1/B1_draft.log
```
...
执行第 4.3 和 4.4 节的 benchmark，只把标签和输出文件改为 `I12-S1-B1`：

```bash
--tag I12-S1-B1-16k-c8 \
--output-file results/innovation12_step1/B1_16k_c8.jsonl
```
...
grep -Ei 'mem_fraction_static|max_total|memory pool|kv cache|out of memory|oom' \
  logs/innovation12_step1/B1_draft.log
```
...
python scripts/specstream/summarize_specstream_profile.py \
  'profiles/innovation12_step1/*.csv' \
  > results/innovation12_step1/profile_summary.tsv

python scripts/specstream/summarize_benchmarks.py \
  'results/innovation12_step1/*.jsonl' \
  > results/innovation12_step1/benchmark_summary.tsv
```
...
5. 16K/30K 长输入的 profile 中 `cpu_history_bytes` 和 `h2d_ops` 大于 0，证明创新点一确实运行。
